...
const mintTrolls = async (web3, numberOfTokens, contractAddress) => {
  ...
  if (web3){
    ...
    const amount = ethers.utils.BigNumber.from(numberOfTokens).mul(ethers.utils.BigNumber.from(10).pow(14)).mul(ethers.utils.BigNumber.from(15));
    console.log(amount)
    const mintTxn = await trollTownContract.mint(numberOfTokens, {value: ethers.utils.formatUnits(amount)});
